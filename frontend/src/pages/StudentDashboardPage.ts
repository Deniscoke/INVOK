...
function profileCard(alias: string, certUnlocked: boolean): string {
  ...
  const parentBtn = `<button type="button" class="btn btn--ghost btn--sm" data-parent-btn>${'\u{1F4C4}'} Rodičom</button>`;
  ...
  <section class="card">
    <div class="identity">
      ...
      <div style="margin-left:auto;display:flex;gap:8px;flex-wrap:wrap">${portfolioBtn}${parentBtn}${certBtn}</div>
    </div>
  </section>`;
}
...
async function loadJourney(alias: string): Promise<void> {
  ...
  slot.querySelector<HTMLButtonElement>('[data-parent-btn]')?.addEventListener('click', () => void openParentReport({ alias }));
  slot
